...
import { Blend, ChevronLeft, ChevronDown, Crop, Info, Pencil, Trash2, Wand2, Image, Ban, SprayCan, UndoDot, UserX, Square, RectangleHorizontal, RectangleVertical, Loader2 } from 'lucide-react';
...
const MediaViewer = ({ resource }: { resource:CloudinaryResources }) => {
  ...
  const tranformations :Transformation ={};

  //setting property to transformation which applies over that component
  if(enhancement === 'restore'){ 
    tranformations.restore = true;
    console.log('restored'); 
  }else if(enhancement === 'improve'){ 
    tranformations.improve = true;
    console.log('improved'); 
  }else if(enhancement === 'remove-background'){ 
    tranformations.removeBackground = true;
    console.log('remvoed-bg'); 
  }
  //for cropping
  if(crop === "square"){
    //to avoid impbalance image size, we consider the smaller part to be the base of img
      if(resource.width > resource.height ){
        tranformations.height =  resource.width;
      }else{
        tranformations.width =  resource.height;
      }
      tranformations.crop = {
        source : true,
        type : 'fill',
    ...
    //considering the aspect ratio of 16:9 + the lib uses g_auto (gravity auto) to sense the main part & focus it
    tranformations.height = Math.floor(resource.width / (16/9));
    tranformations.crop = {
      source : true,
      type : 'fill',
    }
  }else if ( crop === "portrait"){
    tranformations.width = Math.floor(resource.height / (16/9))
    tranformations.crop = {
      source : true,
      type : 'fill',
    }
  ...
  if(typeof filter === 'string' && ['grayscale','sepia'].includes(filter)){
    tranformations[filter as keyof Transformation] = true;
  }else if(typeof filter === 'string' && ['sizzle','quartz'].includes(filter)){
    tranformations.art = filter;
  }

  const hasTransformtion = Object.entries(tranformations).length >0;
  ...
  const canvasHeight = tranformations.height || resource.height;
  const canvasWidth = tranformations.width || resource.width;
  ...
  function closeMenus() {
   ...
   /**
   * closeMenus
   * @description Function to reset the transformation
   */
function disCardChanges(){
  ...
  function handleOnDeletionOpenChange(isOpen: boolean) {
   ...
   /**
   * handle save to save image after enhancement
   */
   async function handleSave() {
    try {
      ...
      // Await the URL fetch to ensure it completes before the next step
      const fetchUrlResponse = await fetch(url);
      if (!fetchUrlResponse.ok) {
        throw new Error('Error fetching the image URL');
      }
  
      // Send the transformed URL and publicId to the server
      ...
      inValidateQueries();
      closeMenus();       //to close the right menu
      disCardChanges();    //to reset
      setVersion(Date.now());

      const resultData = await results.json();
      if (!results.ok) {
        throw new Error(resultData.message || 'Error uploading the image');
      }
      ...
      <Sheet modal={false} open={filterSheetIsOpen}>
        ...
        >
          <Tabs defaultValue="account">
            ...
            <TabsContent value="enhance">
              ...
              <ul className="grid gap-2">
                <li>
                  <Button variant="ghost" className={`text-left justify-start w-full h-14 border-4 bg-zinc-700 
                ...
                <li>
                  <Button variant="ghost" className={`text-left justify-start w-full h-14 border-4 bg-zinc-700 
                  ${enhancement === 'improve' ?'border-white' : 'border-transparent'}`}
                ...
                <li>
                  <Button variant="ghost" className={`text-left justify-start w-full h-14 border-4 bg-zinc-700 
                  ${enhancement === 'restore' ? 'border-white' : 'border-transparent'}`}
                  onClick={()=>setEnhancement('restore')}
                  >
                    <UndoDot className="w-5 h-5 mr-3" />
                    <span className="text-[1.01rem]">Restore</span>
                  </Button>
                </li>
                <li>
                  <Button variant="ghost" className={`text-left justify-start w-full h-14 border-4 bg-zinc-700 
                  ${enhancement === 'remove-background' ?'border-white' : 'border-transparent'}`}
            ...
            <TabsContent value="crop">
              ...
              <ul className="grid gap-2">
                <li>
                  <Button variant="ghost" className={`text-left justify-start w-full h-14 border-4 bg-zinc-700 ${!crop ?'border-white' : 'border-transparent'}`}
                ...
                <li>
                  <Button variant="ghost" className={`text-left justify-start w-full h-14 border-4 bg-zinc-700 ${crop === 'square' ?'border-white' : 'border-transparent'}`}
                ...
                <li>
                  <Button variant="ghost" className={`text-left justify-start w-full h-14 border-4 bg-zinc-700 ${crop === 'landscape' ?'border-white' : 'border-transparent'}`}
                ...
                <li>
                  <Button variant="ghost" className={`text-left justify-start w-full h-14 border-4 bg-zinc-700 ${crop === 'portrait' ?'border-white' : 'border-transparent'}`} 
            ...
            <TabsContent value="filters">
              ...
              <ul className="grid grid-cols-2 gap-2">
                ...
                <li>
                  <button className={`w-full border-4 ${filter === 'quartz' ?'border-white' : 'border-transparent'}`}
                   onClick={()=> setFilter('quartz')}
                  >
                    <CldImage
                      width={156}
                      height={156}
          ...
          <SheetFooter className="gap-2 sm:flex-col">
            ...
            <Button
              variant="outline"
              className={`w-full h-14 text-left justify-center items-center ${hasTransformtion ? 'bg-red-500' : 'bg-transparent'} border-zinc-600`}
              onClick={() => {closeMenus();disCardChanges()}}
            >
              <span className="text-[1.01rem]">
               {hasTransformtion ? "Cancel" : "Close"}
              </span>
            </Button>
          </SheetFooter>
        </SheetContent>
