...
import { Plus, X, Save, Loader2, SquareStack, LayoutDashboard } from 'lucide-react';
...
const MediaGallery = ({ resources : initialResources,tag }: MediaGalleryProps) => {
  ...
  ///collage function 
  function handleOnCreationCollage(){
    setCreation({
      state : 'created',
  ...
  async function handleSavetoLib (){
    ...
    addResources([data]); ///pushing the data to library to show after completion
    setCreation(undefined); //reset the creation 
    setSelected([]); //clearing all the selected img
  }

  //for animation 
  function handleOnAnimation() {
    setCreation({
      state : 'created',
      ...
      <Dialog open={!!creation} onOpenChange={handleOnCreationOpenChange}>
        <DialogContent>
          ...
          <DialogFooter className="justify-end sm:justify-end">
            <Button onClick={handleSavetoLib}>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56">
                  <DropdownMenuGroup>
                    {selected.length > 1 && (
                    <DropdownMenuItem
                    onClick={handleOnCreationCollage}>
